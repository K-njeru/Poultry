  ...
  if (isset($_SESSION["cart_item"])) {
      foreach ($_SESSION["cart_item"] as $item) {
          ...
          $product = $result->fetch_assoc();
              ...
              // Insert order details into tblorders
              $query = $con->prepare("INSERT INTO tblorders (ProductId, Quantity, InvoiceNumber, CustomerName, CustomerContactNo, PaymentMode) VALUES (?, ?, ?, ?, ?, ?)");
              $query->bind_param("iiisss", $pid, $qty, $invoiceno, $cname, $cmobileno, $pmode);
              $query->execute();

              // Reduce available units in tblproducts
              $newunits = $product['Units'] - $qty;
              $update_query = $con->prepare("UPDATE tblproducts SET Units = ? WHERE id = ?");
              $update_query->bind_param("ii", $newunits, $pid);
    ...
    case "add":
      if(!empty($_POST["quantity"])) {
          ...
          if($quan <= $productByCode['Units']) {
              ...
              echo '<script>alert("Units not available. Available Units: ' . $productByCode['Units'] . '")</script>'; 
          }
      }
      break;
